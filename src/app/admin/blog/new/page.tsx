...
export default function AddBlogPostPage() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await fetch('/api/blog?getCategories=true');
        const data = await res.json();
        if (data.categories) {
          setCategories(data.categories);
        } else if (data.success && data.data) {
          // Fallback: try extracting categories from posts
        }
      } catch {
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="text" name="title" value={formData.title} onChange={handleChange} required
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-pink-500" placeholder="Enter post title..." />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Excerpt</label>
              <textarea name="excerpt" value={formData.excerpt} onChange={handleChange} rows={2}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-pink-500" placeholder="Brief summary shown in post listings..." />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Content *</label>
              <textarea name="content" value={formData.content} onChange={handleChange} rows={15} required
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-pink-500 font-mono text-sm" placeholder="Write your blog post content here... (HTML is supported)" />
              <p className="text-xs text-gray-400 mt-1">HTML formatting is supported. Use &lt;p&gt;, &lt;h2&gt;, &lt;strong&gt;, &lt;em&gt;, &lt;ul&gt;, &lt;li&gt; etc.</p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="text" name="metaTitle" value={formData.metaTitle} onChange={handleChange}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-pink-500" placeholder="SEO title (defaults to post title)" />
            </div>
            <div>
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={loading}
              ...
              {loading ? (
                ...
                <><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg> {formData.status === 'PUBLISHED' ? 'Publish Post' : 'Save Draft'}</>
              )}
            </button>
          </div>
        </div>
